...
public class StageService {
    ...
    @Transactional
    public Page<StageDto> getStageStudentPending(long studentId, int page, int size, String session) {

        Pageable pageable = PageRequest.of(page, size);
        return stageRepository.findAllByStudentPendingPage(studentId, session, pageable).map(
                stage -> new StageDto(stage.getId(), stage.getStudent().getId(), new InternOfferDto(stage.getOffer()), stage.getStateStudent(), stage.getStateEmployeur(), stage.getContract() != null ? stage.getContract().id : 0)
        );
    }
