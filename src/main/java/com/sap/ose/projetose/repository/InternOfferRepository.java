...
public interface InternOfferRepository extends JpaRepository<InternOffer, Long> {
    ...
    @Query("SELECT i.state, COUNT(i) FROM InternOffer i WHERE (:session IS NULL OR :session = '' OR i.session = :session) GROUP BY i.state")
    ...
    @Query("SELECT i FROM InternOffer i WHERE i.id = ?1")
    InternOffer getInternOfferById(long id);
}
